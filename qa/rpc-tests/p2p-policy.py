...
class P2PPolicyTests(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # a Bit Core node that behaves similar to mainnet policies
    ...
    def run_test(self):
        ...
        # create a bunch of UTXO with seed money from the Bit Core wallet
        for i in range(20):
    ...
    # spend seed money with a key not in the Bit Core wallet.
    def spend_utxo(self, output, expected_size, retries=0):
        # construct the transaction using Bit Core raw tx APIs
        ...
        # and send the tx from there rather than from Bit Core, to test
