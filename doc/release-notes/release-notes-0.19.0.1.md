...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  B1T), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in B1T per 1,000
...
- #16521 Use the default maxfeerate value as B1T/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as B1T/kB (jonatack)
