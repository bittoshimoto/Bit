...
3. All content of the data directory, except for the `bit.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `bit.conf`       | User-defined configuration settings for `bitd` or `bit-qt`; can be specified by `-conf` option
`./`               | `bitd.pid`        | Stores the process ID (PID) of `bitd` or `bit-qt` while running; can be specified by `-pid` option
...
`bit-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
