...
You must install required dependencies to build a basic Bit daemon, optional dependencies may vary according to your requirements.
...
Bit Core's released binaries are built using the "depends system", which
...
According to installed dependencies, the following steps will compile `bitd`, `bit-cli` and `bit-qt`.
...
#### Enable Qt GUI
Create `bit-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Bit Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Bit Core from self-compiled [depends](/depends/README.md).
...
least 1.5 GB of memory available when compiling Bit Core.
