...
BOOST_FIXTURE_TEST_SUITE(bit_tests, TestingSetup)
...
BOOST_AUTO_TEST_CASE(subsidy_test)
{
    ...
    for (int nHeight = 0; nHeight < 600000; nHeight++) {
        ...
        CAmount nSubsidy = GetBitBlockSubsidy(nHeight, params, prevHash);
    ...
    CAmount nConstantSubsidy = GetBitBlockSubsidy(600000, mainParams.GetConsensus(600000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);

    nConstantSubsidy = GetBitBlockSubsidy(700000, mainParams.GetConsensus(700000), prevHash);
    BOOST_CHECK_EQUAL(nConstantSubsidy, 10000 * COIN);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_difficulty_limit)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1e0fffff);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_pre_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1c271b09);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b671062);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_upper)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b4e56b3);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_modulated_lower)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b335358);
}
...
BOOST_AUTO_TEST_CASE(get_next_work_digishield_rounding)
{
    ...
    BOOST_CHECK_EQUAL(CalculateBitNextWorkRequired(&pindexLast, nLastRetargetTime, params), 0x1b6558a4);
}
