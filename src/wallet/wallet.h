...
/*
 * Bit: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * Bit: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    /**
     * Bit: Get a fee targetting a specific transaction speed.
     */
    CAmount GetBitPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * Bit: Get a fee targetting a specific transaction speed.
     */
    static CAmount GetBitPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the Bit dust fee.
     */
