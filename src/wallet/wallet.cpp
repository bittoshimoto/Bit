...
/**
 * Bit: Effective dust limit for the wallet
...
// Bit: MIN_CHANGE as a function of discardThreshold and minTxFee(1000)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /*
                     * Bit: check all outputs against the discard threshold
                ...
                if (nPriority == MINIMUM) {
                    ...
                    nFeeNeeded = GetBitPriorityFee(txNew, nBytes, nPriority);
                }
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    /* Bit: this has been changed from a static MIN_FINAL_CHANGE that
...
{
    // Bit: Add an increased fee for each output that is lower than the discard threshold
    return std::max(minTxFee.GetFee(nTxBytes) + GetBitDustFee(tx.vout, discardThreshold), ::minRelayTxFeeRate.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    if (nFeeNeeded == 0) {
        ...
        // Bit: Drop the smart fee estimate, use GetRequiredFee
        nFeeNeeded = GetRequiredFee(tx, nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // Bit: as we're adapting minTxFee to never be higher than
...
CAmount CWallet::GetBitPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority)
{
    // payTxFee is the user-set global for desired feerate
    return GetBitPriorityFee(tx, nTxBytes, nPriority, payTxFee.GetFee(nTxBytes));
}
CAmount CWallet::GetBitPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority, CAmount targetFee)
{
    CAmount nFeeNeeded = targetFee;
    // User didn't set: use -txconfirmtarget to estimate...
    if (nFeeNeeded == 0) {
        nFeeNeeded = GetBitFeeRate(nPriority).GetFee(nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // Bit: as we're adapting minTxFee to never be higher than
    //           payTxFee unless explicitly set, this should be fine
