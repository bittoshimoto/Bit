...
# PEPE TODO Change these urls to bit.org and add a depends subdomain which will do what this does
# Dependencies
...
Standalone script to perform the gitian build of Bit Core. Perform
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/bit-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/bit-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean bit git directory because of old caching
    if [ -d inputs/bit/ ]; then
        echo "Cleaning Bit directory..."
        rm -rf inputs/bit/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit bit="$COMMIT" --url bit="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
